...
/* ══════════════════════════════════════════════════════════════════════════
   MODO DEMONSTRAÇÃO — carrega _demo.js e troca o backend por dados simulados.
   Desligado por padrão: sem a flag, este bloco não faz absolutamente nada.
   Ligar:    ?demo=1  ·  Desligar: ?demo=0  ·  Entrada: demo.html
   ══════════════════════════════════════════════════════════════════════════ */
(function _modoDemo(){
  try {
    var q = new URLSearchParams(window.location.search);
    if (q.get('demo') === '1') localStorage.setItem('vitae_demo', '1');
    if (q.get('demo') === '0') localStorage.removeItem('vitae_demo');
    if (localStorage.getItem('vitae_demo') !== '1') return;
    // token de mentira só pra passar pelos porteiros das telas
    if (!localStorage.getItem('vitae_token')) localStorage.setItem('vitae_token', 'demo-token');
    // o ?v muda a cada minuto pra o navegador não servir uma versão velha
    var v = Math.floor(Date.now() / 60000);
    document.write('<scr' + 'ipt src="_demo.js?v=' + v + '"></scr' + 'ipt>');
  } catch (_) {}
})();
